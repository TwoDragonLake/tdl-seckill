...
public class UserInterceptor implements RequestInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        // 查询该用户访问记录
        List<UserRequestRecord> requestRecords = redisUtil.listRange(USER_REQUEST_TIMES_PREFIX + mobile, 0,
        ...
        if (requestRecords.size() + 1 >= systemConfig.getIpBlackTimes() && (System.currentTimeMillis()
            ...
            // 清空访问记录缓存
            redisUtil.deleteRedisDataByKey(USER_REQUEST_TIMES_PREFIX + mobile);
            throw new BusinessException("抢购已经结束啦");
        } else {
            ...
            // 如果第一次设定访问次数，则增加过期时间
            redisUtil.rPush(USER_REQUEST_TIMES_PREFIX + mobile, requestRecord);
        }
        return true;
    }
