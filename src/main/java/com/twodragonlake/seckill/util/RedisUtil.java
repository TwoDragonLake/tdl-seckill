...
public class RedisUtil implements InitializingBean {
     ...
     * @return boolean
     */
    public boolean setRedisData(String key, String fieldName, Object object) {
        return setRedisData(key, fieldName, object, -1);
    }
     ...
     * @return boolean
     */
    public <T> boolean setRedisData(String key, T object) {
        return setRedisData(key, object, -1);
    }
     ...
     * @return T
     */
    public <T> T getRedisData(String key, String fieldName, Class<T> clazz) {
        Jedis jedis = null;
     ...
     * @return T
     */
    public <T> T getRedisData(String key, Class<T> clazz) {
        Jedis jedis = null;
     ...
     * @return 成功\失败
     */
    public boolean deleteRedisDataByKey(String key) {
        Jedis jedis = null;
        try {
            jedis = getConnect();
            jedis.del(key);
        } catch (Exception e) {
            logger.error("redis deleteRedisDataByKey data failed!", e);
            return false;
        } finally {
            close(jedis);
        }
    ...
    public Map<String, String> getMapData(String key) {
        ...
        try {
            ...
            logger.error("redis getRedisData map data failed! ", e);
        } finally {
            close(jedis);
        }
        return dataMap;
    ...
    public <T> List<T> listRange(String key, int start, int end, Class<T> clazz) {
        ...
        try {
            ...
            logger.error("redis range data failed , key = " + key, e);
        } finally {
            close(jedis);
        }
     ...
     * @return boolean
     */
    public <T> boolean rPush(String key, T object) {
        Long ret = rPush(key, object, 0);
        return ret > 0;
    }
    ...
    public boolean releaseLock(String key) {
        return deleteRedisDataByKey(key + "_lock");
    }
